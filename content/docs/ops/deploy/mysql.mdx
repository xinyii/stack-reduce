...
例如版本是 CentOS 7 就选择对应版本下载。

![mysql-1.png](/images/ops/deploy/mysql-1.png)

跳过登录，直接下载。

![mysql-2.png](/images/ops/deploy/mysql-2.png)

这里可以不下载到本地电脑，而是直接复制下载链接，下载到服务器，例如：
...
如果找不到 `yum-config-manager` 命令，则可以直接改配置文件 `/etc/yum.repos.d/mysql-community.repo`，将 `enable` 改为 0 或 1：
